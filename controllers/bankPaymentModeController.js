...
exports.createBankSubPaymentMode = async (req, res) => {
  try {
    const { payment_mode, payment_description } = req.body;

    if (!payment_mode || !payment_description) {
      return res.status(400).json({
        success: false,
        message: 'Please provide both payment mode and description'
      });
    }
    ...
    await AuditLog.create({
      ...
      metadata: { 
        ...
        payment_description: paymentMode.payment_description 
      },
      status: 'SUCCESS'
    });
...
exports.updateBankSubPaymentMode = async (req, res) => {
  try {
    ...
    if (payment_description) updates.payment_description = payment_description;
